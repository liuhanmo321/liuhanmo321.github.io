---
permalink: /
title: "About Me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

<p class="hero-status"><i class="fa-solid fa-location-dot"></i> Huawei Singapore &middot; Previously HKUST</p>

I am **Hanmo LIU**, a researcher in the Search and Recommendation Lab, **Huawei Singapore Research Center**. I earned my Ph.D. in Data Science and Analytics from the [Division of Emerging Interdisciplinary Areas (EMIA)](https://emia.hkust.edu.hk/) at **HKUST** in 2026, fortunate to be advised by Prof. [CHEN, Lei](https://cse.hkust.edu.hk/~leichen/) and Prof. [YANG, Can](https://sites.google.com/site/eeyangc/). Previously, I obtained a B.Sc. in Honors Physics and Computer Science from the [Department of Physics](https://physics.hkust.edu.hk/), HKUST, in 2021.

## Research Interests

My research centers on two complementary angles:

- **Model Evolution** — studying how to make models evolve more efficiently and effectively, spanning continual learning on streaming real-world data and self-driven model redesign empowered by large language models.
- **Empirical Science of Large Models** — treating large-model research as an empirical science: starting from phenomena observed on real data, forming and verifying hypotheses, and distilling generalizable principles that guide future research.

<p class="hero-links">
  <a href="mailto:hliubm@connect.ust.hk"><i class="fas fa-fw fa-envelope"></i> Email</a> &middot;
  <a href="https://scholar.google.com/citations?user=7cL-8BkAAAAJ"><i class="ai ai-google-scholar"></i> Google Scholar</a> &middot;
  <a href="https://orcid.org/0000-0002-6471-0226"><i class="ai ai-orcid"></i> ORCID</a> &middot;
  <a href="https://github.com/liuhanmo321"><i class="fab fa-github"></i> GitHub</a>
</p>

<p class="page-anchors">
  <a href="#publications">Publications</a> &middot;
  <a href="#teaching">Teaching</a> &middot;
  <a href="#academic-service">Service</a> &middot;
  <a href="#awards--honors">Awards</a>
</p>

## Publications

\* marks corresponding author.

### Preprints

- **Learning to Compose for Cross-domain Agentic Workflow Generation**  
  Jialiang Wang, Shengxiang Xu, <u>Hanmo Liu</u>, Jiachuan Wang, Yuyu Luo, Shimin Di, Min-Ling Zhang, Lei Chen  
  <span class="venue">Preprint</span> &middot; [Paper](https://arxiv.org/abs/2602.11114)

- **RobustFlow: Towards Robust Agentic Workflow Generation**  
  Shengxiang Xu, Jiayi Zhang, Shimin Di, Yuyu Luo, Liang Yao, <u>Hanmo Liu</u>, Jia Zhu, Fan Liu, Min-Ling Zhang  
  <span class="venue">Preprint</span> &middot; [Paper](https://arxiv.org/abs/2509.21834)

### Conference Papers

- **Beyond Model Base Retrieval: Weaving Knowledge to Master Fine-grained Neural Network Design**  
  Jialiang Wang, <u>Hanmo Liu</u><sup>*</sup>, Shimin Di, Zhili Wang, Jiachuan Wang, Lei Chen, Xiaofang Zhou  
  <span class="venue">ICML 2026</span> &middot; [Paper](https://arxiv.org/abs/2507.15336)
  
- **Adaptive Concept Discovery for Interpretable Few-Shot Text Classification**  
  Lifang Zheng, <u>Hanmo Liu</u>, Kani Chen  
  <span class="venue">ICLR 2026</span> &middot; [Paper](https://openreview.net/forum?id=UZBQ7iZzYz)

- **Proficient Graph Neural Network Design by Accumulating Knowledge on Large Language Models**  
  Jialiang Wang, <u>Hanmo Liu</u>, Shimin Di, Zhili Wang, Jiachuan Wang, Lei Chen, Xiaofang Zhou  
  <span class="venue">WSDM 2026 · Oral</span> &middot; [Paper](https://arxiv.org/abs/2408.06717)

- **A Selective Learning Method for Temporal Graph Continual Learning**  
  <u>Hanmo Liu</u>, Shimin Di, Haoyang Li, Xun Jian, Yue Wang, Lei Chen  
  <span class="venue">ICML 2025</span> &middot; [Paper](https://proceedings.mlr.press/v267/liu25l.html)

- **Structuring Benchmark into Knowledge Graphs to Assist Large Language Models in Retrieving and Designing Models**  
  <u>Hanmo Liu</u>, Shimin Di, Jialiang Wang, Zhili Wang, Jiachuan Wang, Xiaofang Zhou, Lei Chen  
  <span class="venue">ICLR 2025</span> &middot; [Paper](https://openreview.net/forum?id=49fIu0yDJ4)

- **SIT: Selective Incremental Training for Dynamic Knowledge Graph Embedding**  
  Zhifeng Jia, <u>Hanmo Liu</u>, Haoyang Li, Lei Chen  
  <span class="venue">ICDE 2025</span> &middot; [Paper](https://ieeexplore.ieee.org/document/11113089/)

- **When Speed meets Accuracy: an Efficient and Effective Graph Model for Temporal Link Prediction**  
  Haoyang Li, Yuming Xu, Yiming Li, <u>Hanmo Liu</u>, Darian Li, Chen Jason Zhang, Lei Chen, Qing Li  
  <span class="venue">VLDB 2025</span> &middot; [Paper](https://dl.acm.org/doi/10.14778/3748191.3748203)

- **Effective Data Selection and Replay for Unsupervised Continual Learning**  
  <u>Hanmo Liu</u>, Shimin Di, Haoyang Li, Shuangyin Li, Lei Chen, Xiaofang Zhou  
  <span class="venue">ICDE 2024</span> &middot; [Paper](https://ieeexplore.ieee.org/abstract/document/10598102)

- **Incremental Tabular Learning on Heterogeneous Feature Space**  
  <u>Hanmo Liu</u>, Shimin Di, Lei Chen  
  <span class="venue">SIGMOD 2023</span> &middot; [Paper](https://dl.acm.org/doi/10.1145/3588698)

## Teaching

Teaching Assistant at HKUST:

- **DSAA 5013** — Advanced Machine Learning *(Spring 2024)*
- **DSAA 5020** — Foundation of Data Science and Analytics *(Fall 2023)*
- **MSBD 5002** — Data Mining and Knowledge Discovery *(Spring 2023)*

## Academic Service

- **Program Committee Member** — WWW 2026 Demo, WWW 2025 Short Paper
- **Conference Reviewer** — KDD 2026; ICML 2026; ICLR 2026/2025; MM 2025; WWW 2025; DASFAA 2025/2024
- **Journal Reviewer** — TKDE, TCSS

## Awards & Honors

- DSA Excellence Award (Research) — *DSA Thrust, HKUST(GZ), 2025*
- Second Prize, Outstanding Paper — *Guangdong Computing Association, 2023 / 2024*
- HKUST Postgraduate Scholarship — *HKUST, 2021–2024*
- Paul Ching Wu Chu Scholarship for Outstanding Academic Performance — *Department of Physics, HKUST, 2020*
